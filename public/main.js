document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateUserList = (users) => {
        ...
        users.forEach(user => {
            ...
            li.innerHTML = `<div class="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold" style="background-color: ${generateColor(user)}">${user.charAt(0).toUpperCase()}</div><span class="font-medium">${user}</span>`;
    ...
    const sendMessage = () => {
        ...
        if (message) {
            socket.emit('chat-message', { roomCode: state.currentRoom, message });
            messageInput.value = '';
        }
    };
    ...
    myRoomsList.addEventListener('click', (e) => {
        if (e.target.dataset.roomcode) {
            modalRoomCode.textContent = e.target.dataset.roomcode;
    ...
    socket.on('join-successful', (data) => {
        ...
        if (data.previousMessages) data.previousMessages.forEach(addMessageToUI);
    });
